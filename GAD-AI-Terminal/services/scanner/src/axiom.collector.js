...
exports.discoverAxiomTokens = discoverAxiomTokens;
exports.fetchAxiomMetrics = fetchAxiomMetrics;
...
async function discoverAxiomTokens() {
    if (!API_TOKEN) {
    ...
    const response = await (0, scanner_1.retry)(async () => await axiosPost(url, body));
...
async function fetchAxiomMetrics(mintAddress) {
    if (!API_TOKEN) {
...
async function axiosPost(url, body) {
    return (0, scanner_1.fetchJson)(url, {
        method: 'POST',
        headers: {
