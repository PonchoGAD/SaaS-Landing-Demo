...
function logistic(x: number, midpoint = 0, steepness = 1): number {
...
export function calculateSurvivalScore(input: SurvivalInput): SurvivalResult {
  const {
    ageHours, liquidityUsd, volume24hUsd, holderCount,
    top10HolderPct, rugProbability, aiScore, priceChange1h, txCount1h
  } = input;
  ...
  const explanation =
    overall >= 75 ? `Strong survival outlook — good liquidity ($${(liquidityUsd/1000).toFixed(0)}k) and ${holderCount} holders.` :
