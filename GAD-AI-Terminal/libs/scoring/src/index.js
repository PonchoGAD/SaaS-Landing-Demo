...
exports.calculateFullScore = calculateFullScore;
exports.buildFactorReport = buildFactorReport;
exports.deriveFactors = deriveFactors;
...
function calculateAiScore(factors) {
    const raw = WEIGHTS.growth * factors.growth +
...
function buildExplanation(f, aiScore) {
    const parts = [];
    if (aiScore >= 80)
        parts.push('Strong overall signal.');
    else if (aiScore >= 60)
        parts.push('Moderate signal with upside potential.');
    else if (aiScore >= 40)
        parts.push('Mixed signal — proceed with caution.');
    else
        parts.push('Weak signal — high caution advised.');
    if (f.growth >= 70)
        parts.push('Growth momentum is strong.');
    else if (f.growth < 30)
        parts.push('Growth is stalling.');
    if (f.liquidity >= 70)
        parts.push('Liquidity depth is healthy.');
    else if (f.liquidity < 30)
        parts.push('Low liquidity increases slippage risk.');
    if (f.volume >= 70)
        parts.push('Volume activity is elevated.');
    else if (f.volume < 30)
        parts.push('Volume is thin.');
    if (f.holders >= 70)
        parts.push('Holder base is growing.');
    else if (f.holders < 30)
        parts.push('Holder distribution is weak.');
    if (f.momentum >= 70)
        parts.push('Price momentum is bullish.');
    else if (f.momentum < 30)
        parts.push('Momentum is bearish or flat.');
    if (f.risk >= 70)
        parts.push('Risk score is critical — large wallets may be exiting.');
    else if (f.risk >= 40)
        parts.push('Moderate risk factors present.');
    return parts.join(' ');
}
/** Sprint 3: returns full score object with explanation */
function calculateFullScore(factors) {
...
function buildFactorReport(factors) {
    return {
 ...
 */
function deriveFactors(params) {
    const { priceChange1h, priceChange24h, liquidityChangePercent, volume1h, volume24h, holderCount, holderCountBaseline, riskScore } = params;
    // Growth: weighted price change normalised to 0-100
    const growth = normalizeScore(50 +
        priceChange1h * 0.4 +
        priceChange24h * 0.1);
    ...
    const holderGrowthPct = holderCountBaseline > 0
