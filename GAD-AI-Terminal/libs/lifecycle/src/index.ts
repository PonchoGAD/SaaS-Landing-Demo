...
export interface LifecycleInput {
  ageHours: number;
  volumeAcceleration: number;    // (volume_1h / avg_volume_1h_prev) - 1, ratio e.g. 2.5 = 250% surge
  holderGrowthRate: number;      // new holders per hour
  priceChange1h: number;         // % price change last hour
  priceChange24h: number;        // % price change last 24h
  whaleNetFlow: number;          // positive = whales buying, negative = selling (0-100 scale)
  socialAcceleration: number;    // hype score velocity (0-100)
  liquidityUsd: number;
  sellBuyRatio: number;          // sell volume / buy volume (>1 = more selling)
...
export interface LifecycleResult {
  ...
  stageScore: number;            // 0-100 confidence in this stage
  nextStage: LifecycleStage | null;
  timeInStageEst: string;        // estimated time in current stage
  factors: {
  ...
  explanation: string;
  alert: string | null;          // notable transition signal
}
...
function computeStageScores(input: LifecycleInput): Record<LifecycleStage, number> {
  const {
    ageHours, volumeAcceleration, holderGrowthRate, priceChange1h,
    priceChange24h, whaleNetFlow, socialAcceleration, liquidityUsd,
    sellBuyRatio, rugProbability, holderCount
  } = input;
  ...
  scores.ACCUMULATION = clamp(
    (holderGrowthRate > 5 && holderGrowthRate < 100 ? 35 : 0) +
  ...
  scores.BREAKOUT = clamp(
    (volumeAcceleration > 2 ? 40 : volumeAcceleration > 1 ? 20 : 0) +
  ...
  scores.HYPE = clamp(
    (socialAcc > 70 ? 40 : socialAcc > 50 ? 20 : 0) +
  ...
  scores.DISTRIBUTION = clamp(
    (sellBuyRatio > 1.3 ? 40 : sellBuyRatio > 1.1 ? 20 : 0) +
...
const STAGE_NEXT: Record<LifecycleStage, LifecycleStage | null> = {
  BIRTH:        'ACCUMULATION',
...
const STAGE_EST_TIME: Record<LifecycleStage, string> = {
  BIRTH:        '0–6 hours',
...
const STAGE_ALERTS: Partial<Record<LifecycleStage, string>> = {
...
export function detectLifecycle(input: LifecycleInput): LifecycleResult {
  const scores = computeStageScores(input);

  // Winning stage = highest score
  const sorted = (Object.entries(scores) as [LifecycleStage, number][])
    .sort((a, b) => b[1] - a[1]);
...
function buildExplanation(
  ...
  sorted: [LifecycleStage, number][]
): string {
  const second = sorted[1];
  const confidence = score - second[1];

  const parts: string[] = [`Stage: ${stage} (confidence ${Math.round(score)}/100).`];
...
export function lifecycleBonus(stage: LifecycleStage): number {
  const bonuses: Record<LifecycleStage, number> = {
...
export function lifecycleEmoji(stage: LifecycleStage): string {
