...
export interface ReputationInput {
  // Performance metrics
  winRate: number;              // 0-100: % of trades that were profitable
  totalReturn: number;          // overall ROI %
  verifiedWins: number;         // # of 5x+ wins
  verifiedRugs: number;         // # of times caught in rugs
  avgHoldHours: number;         // average hold time in hours

  // Timing quality
  earlyEntryRate: number;       // 0-1: how often buys at BIRTH/ACCUMULATION stage
  exitQuality: number;          // 0-100: exits before dump? (high = smart exits)

  // Conviction signals
  avgConvictionX: number;       // avg unrealized gain when they hold (higher = stronger conviction)
  maxConvictionHoldHours: number; // longest hold of a winner
...
export interface ReputationResult {
  ...
  description: string;
  warning: string | null;       // if EXIT_LIQUIDITY or TOURIST
}
...
export function classifyReputation(input: ReputationInput): ReputationResult {
  // ─── Component scores ──────────────────────────────────────────────────────

  // Win rate (0-100): >70% = LEGEND territory
  const winRateScore = clamp(
    input.winRate >= 70 ? 100 :
  ...
  const reputationScore = clamp(Math.round(
    winRateScore    * 0.25 +
...
/** Emoji for tier */
export function reputationEmoji(tier: ReputationTier): string {
...
export function reputationWeight(tier: ReputationTier): number {
  const weights: Record<ReputationTier, number> = {
    LEGEND:         2.0,
    SMART:          1.5,
    AVERAGE:        1.0,
    TOURIST:        0.5,
