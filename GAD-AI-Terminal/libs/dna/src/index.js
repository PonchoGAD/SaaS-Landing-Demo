...
exports.DNA_TYPES = void 0;
exports.classifyWalletDna = classifyWalletDna;
exports.aggregateTokenDna = aggregateTokenDna;
exports.DNA_TYPES = ['SNIPER', 'WHALE', 'INSIDER', 'HOLDER', 'SCALPER', 'SWING', 'DEGENERATE', 'UNKNOWN'];
function classifyWalletDna(a) {
    if (a.totalTrades < 3) {
        return {
            dnaType: 'UNKNOWN',
    ...
    scores.SNIPER = Math.min(100, Math.round(a.earlyBuyRatio * 50 +
        (a.sniperTradeCount / Math.max(1, a.totalTrades)) * 30 +
        (a.avgHoldMinutes < 10 ? 20 : a.avgHoldMinutes < 60 ? 10 : 0)));
    // WHALE: large positions
    scores.WHALE = Math.min(100, Math.round(Math.log10(Math.max(1, a.avgPositionUsd)) / Math.log10(100000) * 100));
    // INSIDER: buys before pumps
    scores.INSIDER = Math.min(100, Math.round((a.insiderSignals / Math.max(1, a.totalTrades)) * 150));
    // HOLDER: long average hold
    scores.HOLDER = Math.min(100, Math.round(a.avgHoldMinutes > 10080 ? 100 : // >7 days
        a.avgHoldMinutes > 1440 ? 75 : // >1 day
            a.avgHoldMinutes > 360 ? 50 : 20));
    // SCALPER: many small trades, short holds
    scores.SCALPER = Math.min(100, Math.round((a.totalTrades > 100 ? 40 : a.totalTrades > 30 ? 20 : 5) +
        (a.avgHoldMinutes < 5 ? 40 : a.avgHoldMinutes < 30 ? 20 : 0) +
        (a.avgPositionUsd < 500 ? 20 : 0)));
    // SWING: medium holds, decent win rate
    scores.SWING = Math.min(100, Math.round((a.avgHoldMinutes >= 60 && a.avgHoldMinutes <= 10080 ? 40 : 0) +
        (a.winRate >= 55 ? 30 : a.winRate >= 45 ? 15 : 0) +
        (a.roi > 20 ? 30 : a.roi > 0 ? 15 : 0)));
    // DEGENERATE: high risk, many trades, volatile
    scores.DEGENERATE = Math.min(100, Math.round((a.totalTrades > 200 ? 40 : a.totalTrades > 50 ? 20 : 0) +
...
function aggregateTokenDna(buyerDna) {
    const totals = {};
    for (const d of buyerDna)
