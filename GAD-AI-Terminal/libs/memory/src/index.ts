...
export interface SimilarityResult {
  similarityScore: number;       // 0-100 overall similarity to winners
  matchedWinners: number;        // how many winner records match
  avgWinnerGainX: number;        // avg gain of matched winners
  topMatchOutcome: string | null;
  rugSimilarity: number;         // 0-100 similarity to PAST RUGS (bad if high)
  explanation: string;
}
...
function buildFeatureVector(snap: TokenSnapshot): number[] {
  return [
...
function narrativeBonus(tagA: string, tagB: string): number {
  return tagA === tagB ? 0.1 : 0;
}

// Lifecycle stage bonus
function lifecycleBonus(stageA: string, stageB: string): number {
  return stageA === stageB ? 0.05 : 0;
}

const SIMILARITY_THRESHOLD = 0.65; // minimum cosine to count as "match"

export function calculateAlphaSimilarity(
  ...
  let winnerMatches: Array<{ sim: number; gainX: number; outcome: string }> = [];
  let rugMatches: Array<{ sim: number }> = [];
...
export function buildTokenSnapshot(params: {
  aiScore: number;
  riskScore: number;
...
export function labelOutcome(peakGainX: number): HistoricalRecord['outcome'] {
  if (peakGainX <= 0)   return 'DEAD';
  if (peakGainX < 2)    return 'NEUTRAL';
  if (peakGainX < 10)   return 'NEUTRAL';
  if (peakGainX < 50)   return 'WINNER_10X';
  if (peakGainX < 100)  return 'WINNER_50X';
  return 'WINNER_100X';
}
