...
exports.REGIMES = void 0;
exports.detectMarketRegime = detectMarketRegime;
exports.regimeFromDb = regimeFromDb;
exports.regimeEmoji = regimeEmoji;
exports.REGIMES = ['BULL', 'BEAR', 'SIDEWAYS', 'EUPHORIA', 'PANIC'];
function clamp(v, lo = 0, hi = 100) {
    return Math.min(hi, Math.max(lo, v));
}
function detectMarketRegime(input) {
    const { solPriceChange7d, solPriceChange24h, btcPriceChange7d, totalMarketVolume24h, avgVolume7d, fearGreedIndex, newTokensPerHour, rugRatePercent, avgGadScore } = input;
    // ─── Signal scores ─────────────────────────────────────────────────────────
    // Momentum: positive = bullish
    const momentumScore = solPriceChange7d * 0.4 +
    ...
    regimeScores.BEAR = clamp((momentumScore < -5 ? 40 : momentumScore < 0 ? 20 : 0) +
    ...
    regimeScores.PANIC = clamp((fearGreedIndex < 20 ? 40 : 0) +
    ...
    const sorted = Object.entries(regimeScores)
        .sort((a, b) => b[1] - a[1]);
...
function regimeFromDb(row) {
    const regime = exports.REGIMES.includes(row.regime)
        ? row.regime
        : 'SIDEWAYS';
    const MULTIPLIERS = {
