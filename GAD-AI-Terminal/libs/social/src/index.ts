...
export interface SocialData {
  ...
  engagementRate: number;  // 0-1 (avg likes+retweets / followers)
  sentimentScore: number;  // 0-1 (0=bearish, 0.5=neutral, 1=bullish)
  followerGrowthPct: number; // % follower growth in 24h
...
export function calculateHypeScore(data: SocialData): HypeResult {
  ...
  const mentionScore = Math.min(100, Math.round(
    Math.log10(Math.max(1, hourlyRate)) / Math.log10(10000) * 100
  ));
  ...
  const communityBonus = Math.min(20, Math.round(
    Math.log10(Math.max(1, data.telegramMentions + data.pumpFunComments)) * 5
  ));

  // Follower growth bonus
  const growthBonus = Math.min(10, Math.round(data.followerGrowthPct * 0.5));

  const hypeScore = Math.min(100, Math.max(0, Math.round(
    velocityScore * 0.35 +
    mentionScore  * 0.25 +
...
export function buildSocialDataFromMetrics(params: {
  volume5m: number;
  volume1h: number;
  txCount5m: number;
  txCount1h: number;
